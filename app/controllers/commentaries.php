...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['goComment'])) {
    ...
    } elseif (mb_strlen($comment, 'UTF8') < 7) {
        array_push($errMsg, "Комментарий должен быть длинее 7 символов");
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_comment'])) {
    ...
    if ($text === '') {
        ...
        array_push($errMsg, "Количество символов внутри комментария меньше 7");
    } else {
        $com = [
            'comment' => $text,
            'status' => $publish
